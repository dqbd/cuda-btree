...
\begin{algorithm}
  ...
  \SetKwFunction{AllocateNode}{AllocateNode}
  ...
  \Fn{\SplitNode{$x$}}{
    ...
    $x_1 \gets \AllocateNode{}$\;
    $x_1.keys \gets (x.key_1, \dots, x.key_{median - 1})$\;
    $x_1.child \gets (x.child_1, \dots, x.child_{median})$\;

    $x_2 \gets \AllocateNode{}$\;
    $x_2.keys \gets x.key_{median + 1}, \dots, x.key_{x.size}$\;
    $x_2.child \gets x.child_{median + 1}, \dots, x.child_{x.size + 1}$\;

    \KwRet $(x_1,\ x.key_{median},\ x_2)$\;
  }

  \Fn{\Insert{$root$, $key$}}{
    $x \gets root$\;
    \While{$x \neq null$}{
      \If{$\exists i$, $x.key_i = key$}{
        \Break
      }

      $i \gets i$, which points to the first key where $x.keys_i > key$
    }
  }

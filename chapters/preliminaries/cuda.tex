...
Kernel launches from the host, for example, are asynchronous by nature, and the host code will not wait for results unless \mintinline{cpp}{cudaDeviceSynchronize()} is invoked. This instruction will block the host until the device has completed all preceding requested tasks \cite{cudaprog}.

Threads can also be synchronized, as threads do not execute at the same time and thus do not complete their task simultaneously. A synchronization barrier \mintinline{cpp}{__syncthreads()} is used to synchronize the threads in a thread block, which all threads must reach to resume execution. This barrier guarantees that all code preceding the instruction is executed before the code after the instruction.

Threads inside a thread block may use shared memory or global memory to cooperate and share data. To ensure all writes to the memory are visible to all other threads, \mintinline{cpp}{__threadfence()} can be used to stall the threads until writes to the memory are completed.

Warp-level primitives enable fine-tuned collective operations within warps. Parallel reductions and synchronized data exchange can be achieved with \mintinline{cpp}{__shfl_down()} and \mintinline{cpp}{__shfl_sync()} respectively. Similar to \mintinline{cpp}{__syncthreads()}, \mintinline{cpp}{__syncwarp()} can be utilized to create a synchronization barrier for all threads in a warp.

At last, atomic functions (such as \mintinline{cpp}{__atomicAdd()} or \mintinline{cpp}{__atomicCAS()}) can be used to read from or write to shared or global memory while ensuring no interference from other threads until the operation is complete.